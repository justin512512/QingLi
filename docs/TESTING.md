...
- `scripts/package.ps1 -PortableOnly` 先运行测试，测试失败时立即退出。
- 打包脚本生成 `artifacts/QingLi-<version>-win-x64-portable.zip`。
- ZIP 必须包含 `QingLi.Windows.exe`、`QingLi.Recovery.exe`、节假日包和 366 天历史事件包。
...
- 全新安装或启动后，无需联网即可打开月历。
- 单击任务栏时间日期整个区域可打开约 900×440 的三栏信息日历。
- 左栏显示“历史上的今天”，中栏显示农历、干支生肖、宜忌和休班标记，右栏显示未来事件倒计时。
- 可以新增公历生日；退出并重启后仍存在。
- 可以新增农历生日；退出并重启后仍存在。
- 可以新增公历/农历纪念日；退出并重启后仍存在。
- 到期提醒同一天只出现一次。
